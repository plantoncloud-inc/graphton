...
class TestRemoteDeploymentSimulation:
    ...
    async def test_config_parameter_extraction(self) -> None:
        ...
        with patch('graphton.core.middleware.load_mcp_tools') as mock_load:
            ...
            try:
                result = await middleware.abefore_agent(state={}, runtime=config)
    ...
    async def test_missing_config_raises_clear_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="requires template variables"):
            await middleware.abefore_agent(state={}, runtime=None)  # type: ignore[arg-type]
    
    async def test_missing_configurable_raises_clear_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="requires template variables"):
            await middleware.abefore_agent(state={}, runtime=config)  # type: ignore[arg-type]
    
    async def test_missing_token_in_config(self) -> None:
